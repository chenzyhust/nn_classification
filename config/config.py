...
# global settings
_C.workspace           = "/skin_data/commodity/models/0514/train_4/shufflenetv2_x1_0_96" # directory to save weights file
_C.date_format         = '%A_%d_%B_%Y_%Hh_%Mm_%Ss' # time of we run the script
_C.time_now            = datetime.now().strftime(_C.date_format)
_C.net                 = "shufflenetv2_x1_0" # net type
_C.local_rank          = 0
...
# dataset parameters
_C.dataset                   = CN()
_C.dataset.train_path        = "/skin_data/commodity/train_cascade/train.txt"
_C.dataset.test_path         = "/skin_data/commodity/train_cascade/test.txt"
_C.dataset.img_width         = 96
_C.dataset.img_height        = 96
_C.dataset.num_workers       = 4
_C.dataset.train_transforms  = [
  transforms.ToPILImage(),
  transforms.Resize([_C.dataset.img_width, _C.dataset.img_height]),
  transforms.RandomCrop((_C.dataset.img_width, _C.dataset.img_height), 12),
...
# train parameters 
_C.train = CN()
_C.train.classes     = 4               # num of classes
...
_C.train.gpu_id      = "0,1"         # id(s) for CUDA_VISIBLE_DEVICES
...
_C.train.loss        = "LabelSmoothingLoss" # ["CrossEntropyLoss", "LabelSmoothingLoss","FocalLoss", "CenterLoss"]
